...
Одна сессия берет роль оркестратора и ведет борд: цель, задачи, факты снаружи. Рабочие сессии берут, закрывают и блокируют задачи. Каждый ход по борду будит оркестратора: он раздает следующую задачу, разбирает блокировки или спрашивает человека диалогом поверх всех окон. Он не поллит и сам сессий не запускает: в приложении он оставляет чип, который открывает сессию по клику человека, в терминале дает первую строку для вставки.
