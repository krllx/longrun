...
macOS или Linux, Claude Code, python3 3.9+; больше ничего. Привычнее из клона? Тот же [`install.sh`](install.sh).
...
Дальше открыть проект в Claude Code и сказать **"настрой longrun"**: агент сделает папку проектом и пройдет с вами по настройкам, по одной. То же из терминала:

```bash
cd ~/my-project && longrun init && claude "настрой longrun"
```

Уже открытые проекты и сессии тоже считаются: `longrun init` в папке или "настрой longrun" в той сессии подключает их без перезапуска - хуки работают сразу, дайджест заметок придет на следующем старте сессии.
