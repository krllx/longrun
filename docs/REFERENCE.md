...
| `UserPromptSubmit` | turn counter; finishing reading the app metadata and re-binding the chain; under HQ every 5 turns - `@user` items and overdue `next:` | messages from the inbox; own notes once on re-binding; the shared notes delta; the turn card: what the turn that just ended changed and what failed in it, at most once per 8 turns, only when it edited or failed something and wrote nothing down |
