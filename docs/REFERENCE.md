...
| `UserPromptSubmit` | turn counter; finishing reading the app metadata and re-binding the chain; under HQ every 5 turns - `@user` items and overdue `next:` | messages from the inbox; own notes once on re-binding; the shared notes delta; a reminder once a window of 8 turns has passed since the last note or reminder, and only after edits or a failed command |
| `PostToolUse` (Edit, Write, MultiEdit, NotebookEdit, Bash, PowerShell, Agent, Task, Workflow, mcp__*) | counters; only the four editing tools count as edits; the list of edited files | messages from the inbox as `additionalContext`; the same reminder, on a window of 40 calls |
...
| `nudge_turns`, `nudge_tools`, `nudge_edit_tools` | 8, 40, 6 | the reminder window and the minimum number of edits for it. The window is measured from where the counters stood at the last reminder, not taken modulo: a counter that steps OVER the boundary (`PostToolUseFailure` charges one without checking the reminder) used to skip a whole window |
...
| `notify setup [--icon FILE]` | build the bundle notifications are sent from, so they carry our icon and the name `longrun` instead of terminal-notifier's (macOS only; `install.sh` runs it when you say yes to its question) |
