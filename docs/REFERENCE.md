...
| `PostToolBatch` (no matcher) | one event per batch of parallel calls, after all of them resolve. Two things only this event can know: the files the batch opened, Reads included, and the size of the turn counted in the calls `PostToolUse` is *not* fired for - so a turn spent entirely in `Read` stops measuring zero and the read-only card can see it. Each call is counted by exactly one of the two events. It does **not** carry the recall hint: every tool holding a search term is already in the `PostToolUse` matcher, so a second pass read the same files twice and spent the same (term, file) pair again, on output the CLI may drop when a turn ends on a tool result. Newer than the other events: where the CLI does not know it, the entry is inert, the hint is unaffected and only the two counts above are lost | - |
...
The reference for "after its description" is what the file was at the moment `doc add` or `doc touch` ran - its mtime, its size and a short hash of its contents - kept per entry in `<project>/docs.json`. It used to be the note's own date, and a note carries no time of day, so a change made hours later on the **same day** read as no change at all: exactly the case that matters, another window rewriting the material while this session was closed. The size and the hash are there because mtime alone cannot tell a rewrite from a **branch switch**: `git checkout` restamps every file it touches, so with mtime alone every doc pointer carried a stale warning after every switch, including one that put back the very bytes the description was written against. The hash is only ever computed once the mtime has moved, so the steady state stays one `stat` per entry, and a file over 4 MB is compared by size alone. A project written before `docs.json` existed has no stamps and falls back to the old day-granularity comparison rather than going quiet. `gc` drops stamps whose pointer is gone.

A pointer whose file is missing is archived after `DOC_MISSING_DAYS`, counted from the moment a `gc` **first found the file gone**, stamped into the same entry. It used to be counted from the date of the note, which is a different thing entirely: a pointer written a month ago whose file was deleted this morning was archived by the very next hourly `gc`, and putting the file back did not bring the pointer back. A file that reappears clears the clock.
...
**A pattern and a sentence are not the same signal**, and treating them as one was the mistake 0.6.2 shipped. A `Grep`/`Glob` pattern is the string the session typed to find something: one word of it *is* the question and stands on its own wherever it is found, several words of it are a co-occurrence the session asked for and stand together. A subagent's `description` or a web query is a sentence, and most of a sentence is there to be a sentence - so only the words in it that **name** something count. A word names something when it carries an underscore, a digit or an internal capital, or when it is simply 12 characters or longer: `CLAUDE_ENV_FILE` and `retry_budget_ms` name the question, `native` and `review` are English. Pairing two ordinary words is no help either, because a shared note runs to 400 characters and almost any English sentence has two of its words somewhere inside one.

In a notes file, only the tags another session is owed count on top of that: `dead`, `decision`, `fact`, `pin`, `doc`. `ctx` and `todo` are one conversation's own bookkeeping - a single archived `ctx` note was the source of a third of all the noise in the first version.

Four more things keep the volume down: a term must be at least 5 characters; a term found on more than 6 lines is a word rather than a term; the same (term, file) pair is never pointed at twice; and at most two hints fire per turn, the budget refilling at the turn boundary. One pass over the material is capped at 600 kB whatever the project holds. The hint quotes at most two lines and hands over to `longrun recall <term>` for the rest, and the quote is filed under the term that *carried* the line rather than one that rode along on it.

What that is worth, measured three ways on this project (`research/measure-recall.py`), 0.6.2 against 0.6.3:

| | 0.6.2 | 0.6.3 |
|---|---|---|
| reach: a one-word `Grep` for the most specific word of a stored line brings the line back | 58/271 (21%) | **188/271 (69%)** |
| false positives: real `Grep`/`Glob` patterns from unrelated projects | 2/197 (1.0%) | **2/197 (1.0%)** |
| real subagent descriptions and web queries of this project | 28/43 (65%) | **0/43 (0%)** |

The middle row is the honest denominator and the first is what the mechanism is for: three times the reach at the same false-positive rate. The third row is the trade, stated plainly - a sentence no longer fires unless it names something. Note what the corpus could not show: this project's own history contains 38 subagent descriptions, 5 web queries and **not one `Grep`**, so the replay that blessed 0.6.2 exercised only the weakest path. `measure-recall.py` now prints the composition of its sample, marks the tools it did not exercise, and measures reach and false positives separately, because a mechanism tuned for quiet is easy to tune into silence and silence looks identical to "no noise" in a firing count.
...
**A turn that changed nothing but did real work.** Added in 0.6.2 and measured before it was built. `research/measure-cards.py` reads every transcript on the machine and asks the question backwards: rather than judging which turns *should* have written a note, it counts the turns that *did*, since a note written unprompted is the strongest evidence a turn was worth asking about. About half of them - 51% on the current corpus - happen in turns the edit gate is silent in, and around 140 have no edit and no failure at all. The gate was aimed at less than half the territory notes come from.

The threshold is 6 tool calls, and the table behind it is now fine enough to say so: the note rate in a read-only turn runs 10-24% below six calls and 36-53% at six and above. It was not fine enough before - with 1-2 / 3-5 / 6-10 buckets, "4" and "6" fell on the same bucket edge and produced identical numbers, so "measured" claimed more than the table could carry. Two other things the script had wrong: it modelled the gate as edits made *in one turn*, while `edit_tools_since_note` accumulates across turns and is reset only by a note write (it now models both and prints the difference), and it counted a command the user interrupted as a command that failed, which `PostToolUseFailure` does not.
...
Eight keys have been retired and are now fixed numbers in the script: `strict_stop` and `auto_init` (both off, and both a mode nobody ran), `journal_tail_lines` (12), `handoff_max_bytes` (1100), `fail_keep` (40), `recall_transcripts` (6), `ctx_sample_every` (5) in 0.6.0, and `compact_hint_max_bytes` (1200) in 0.6.1. A setting that has never been changed is not a setting. What 0.6.2 and 0.6.3 added is fixed for the same reason: the housekeeping question waits 4 turns, the mid-turn delta runs at most once per 5 tool calls, the recall hint allows 2 per turn over terms of 5 characters or more found on at most 6 lines (12 characters being the length at which a plain word names something on its own), the poll hint triggers on a 30-second sleep or a third identical command, the read-only turn card wants 6 tool calls, and a doc file over 4 MB is compared by size instead of by hash. A retired key left in a config.json is ignored, shown as retired by `longrun config`, and removed by `config unset`; `config set` on one says what replaced it instead of "unknown key".
