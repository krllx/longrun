...
- `longrun prune [--auto] [--shared]` - deletion candidates (`todo` is cheapest, then `ctx`, `fact`, `dead`/`decision`, then `doc`; `pin` is never touched); `--auto` archives down to 60% of the budget.
