...
|---|---|
| compaction은 대화 기록을 요약으로 압축하는데, 결정의 이유가 가장 먼저 사라집니다. 왜 그 방법을 버렸는지, 왜 이 길을 택했는지가 남지 않습니다. 30분 뒤 에이전트는 자기가 이미 되돌린 수정을 다시 제안합니다. | **디스크 위의 메모**. 막다른 길, 결정, 사실을 한 줄씩. 공유 메모는 모든 세션이 보고, 자체 메모는 compaction, `/clear`, resume 때마다 돌아옵니다. |
...
### 4. 켜야 동작하는 자율성: 한 세션이 나머지를 이끄는 방식
...
기준은 하나입니다. **명령 한 번, 파일 읽기 한 번, grep 한 번으로 되찾을 수 있는가**? 그렇다면 적지 않습니다.
