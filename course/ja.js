...
(function (C) {
  ...
  steps(C.s1, [
    ...
    ['マイルストーン：メモではなくジャーナルへ', 'エージェント', '進捗（「テストが緑」「PR作成」）はセッションのジャーナルへ入れます。メモは取り戻せない事実のため、ジャーナルは時系列のためのものです。'],
  ...
  steps(C.s2, [
    ...
    ['予算は動かせない：addは拒否し、pruneが片付ける', 'エージェント', '共有メモは5000バイト、自分のメモは3000バイト。一杯のファイルが古い行を黙って追い出すことはありません。<code>add</code>は拒否し、削っても影響の小さいエントリを挙げます。習慣は、終わったものは<code>rm</code>、2行目を足す代わりに<code>replace</code>、ダイジェストにPRUNE NEEDEDと出たら<code>prune</code>です。'],
  ]);
  steps(C.s3, [
