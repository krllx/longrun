...
(function (C) {
  ...
  const PF = {
    ...
    'archive/precompact/<sid>-<time>.md': ['compactionの前のスナップショット：ユーザーの直近のリクエスト、編集したファイル、最近のFAIL。HANDOFFブロック（compaction後の自分への引き継ぎ）はこれから組み立てられます。', 'PreCompact hook'],
  ...
  const OF = {
    ...
    'aaaaaaaa/journal.md': ['マイルストーン (longrun log) と機械的な行：開始、終了、失敗したコマンド、compaction、送信。', 'hookとlongrun log'],
    ...
    'bbbbbbbb/meta.json': ['セッションBの同じもの。', 'hook'],
  ...
  const GF = {
    'watch/w1.json': ['遅延チェック1件：何を確かめ、誰に何を伝え、いつ期限切れになるか。', 'watch add；タイマーのtickが更新する'],
    'halt.json': ['全プロジェクトの全セッションに効く停止：誰が、いつ、なぜ。', 'halt / resume、予算の監視役'],
  ...
  steps(C.s1, [
    ...
    ['セッションAが始まり、SessionStart hookがダイジェストを出す', 'hook', 'hookはツリーを上へたどって.longrun/を探しプロジェクトを見つけ、リポジトリの外にセッションのディレクトリを作り、ダイジェストをモデルのコンテキストへ出力します。いまはまだ空です。'],
    ...
    ['全員に必要な事実：共有メモへ', 'エージェント', 'PR番号と型の決定は、このプロジェクトのどのセッションにも必要です。<code>--shared</code>はそれをプロジェクトのNOTES.mdへ書きます。<b>[n1]</b>、<b>[n2]</b>、著者ラベルつきです。'],
    ['マイルストーン：メモではなくジャーナルへ', 'エージェント', '進捗（「テストが緑」「PR作成」）はセッションのジャーナルへ入れます。メモは導き直せないもののため、ジャーナルは時系列のためのものです。'],
    ['compaction：機械層がスナップショットとアーカイブを取る', 'hook', 'コンテキストがあふれ、Claude Codeが履歴を要約へ押し込めます。<b>PreCompact</b>がスナップショット（ユーザーの直近のリクエスト、編集したファイル、FAIL）を保存し、<b>PostCompact</b>が要約をそのままアーカイブします。モデルはここで何もしません。'],
    ...
    ['細部が足りない：調べ直さずrecall', 'エージェント', '検索は全セッションの共有メモと自分のメモ、各ジャーナル、アーカイブされた要約、そしてプロジェクトのtranscriptに及びます。まずこれ、コードを読み直すのはその後です。'],
  ]);
  steps(C.s2, [
    ['セッションBが同じフォルダで開く', 'hook', 'Bはまだ何もしていませんが、PR番号、Decimalを使う決定、そしてAが隣で作業していることをすでに知っています。それが<b>SESSIONS</b>ブロックです。誰がいるか、生きているかどうか、自分のメモが何件か、それぞれ最後に何をしたか。'],
    ...
    ['誰が何をしているか：status', 'エージェント', 'すべて状態ファイルからで、transcriptは読みません。名前、生きているかどうか、自分のメモ、呼び出し回数、compaction回数、失敗回数、最後の回答。'],
    ['予算は硬い：addは拒否し、pruneが片付ける', 'エージェント', '共有メモは5000バイト、自分のメモは3000バイト。一杯のファイルが古い行を黙って追い出すことはありません。<code>add</code>は拒否し、落とすのに一番安いエントリを挙げます。習慣は、終わったものは<code>rm</code>、2行目を足す代わりに<code>replace</code>、ダイジェストにPRUNE NEEDEDと出たら<code>prune</code>です。'],
  ]);
  steps(C.s3, [
    ...
    ['Bが目を覚ます：メッセージがターンとして届く', 'hook', 'Bの最初のhook（ターン、ツール呼び出し、開始のいずれか）が受信箱からファイルを取り、コンテキストへ出力します。モデルにとってこれは人間からではなく仲間からの依頼で、権限は自分のもののままです。'],
    ['Bがいま動いている場合：socket', 'CLI', '動いているセッションはunix socketを聞いています。本文はそこへ送られ、ツール呼び出しの合間にユーザーのターンとしてBに現れます。受信箱は通りません。<code>--resume</code>は<code>claude -p --resume</code>で止まったセッションを起こします。'],
    ['イベントを待つ：ポーリングではなくwatch', 'エージェントA', '「2時間後に見て」「PRがマージされたら」。sleepループも、人間へのリマインドも要りません。登録されるのは決定的なチェックです。<code>add</code>はその場で1度試します。すでに成立していれば何も登録せず、いま動きます。'],
    ['タイマーが5分ごとに確認、モデルなし', 'タイマー', '1回のtickはtokenを使いません。条件が成立しなければ記録を更新して終わりです。ノートPCがスリープしていても、起きたときに追いつきます。ハードエラーが3回続くか期限を過ぎると、"broken"か"expired"のメッセージが1通来ます。'],
    ['条件が成立：宛先へメッセージ', 'タイマー', 'tickは<code>--then</code>の文面を<code>send</code>と同じ経路で送ります。動いているセッションのsocketか、受信箱かです。セッションは"From longrun watch w1"を読み、そのとおりに動きます。その文面を書いたのは自分自身で、しかもcompactionより前かもしれません。'],
  ]);
  steps(C.s4, [
    ['Aがオーケストレーターの役を引き受ける', 'エージェントA、人間の指示で', 'プロジェクトごとに1つのセッションが目標を進めます。役割の一覧を受け取り、<b>何もポーリングしません</b>。起こすのはイベントです。2つ目のセッションは、いま誰が持っているかを添えて断られます。'],
    ['ボード：タスクと割り当て', 'オーケストレーター', 'タスクT<n>の状態はtodo / doing / blocked / done / dropped。<code>assign</code>はタスクを生きているセッションのsocketへ、止まっているセッションなら受信箱へ入れます。'],
    ['Bがタスクをターンとして受け取る', 'hook', '作業役のセッションに必要なコマンドは3つだけ、<code>take</code>、<code>done</code>、<code>block</code>です。ほかに覚えることはありません。'],
    ['Bが作業してタスクを閉じる。外から来た事実', 'エージェントB', '<code>done</code>と<code>block</code>はオーケストレーターを起こします。レビュアーがフィールド名の変更を求めました。これが<b>外から来た事実</b>です。決まりは、まず記録し (<code>fact</code>)、判断はその後。この事実はオーケストレーターにも届きます。'],
    ...
    ['全部止める：halt', 'オーケストレーター、人間の指示で', 'halt.jsonファイルはグローバルです。どのセッションでも<b>PreToolUse</b> hookが理由つきの拒否を返し、モデルはそれを見て、何をしようとしていたかを人間に伝えます。解除できるのは人間だけで、会話の中で言葉にして行います。<code>longrun resume</code>です。'],
    ['全ウィンドウの手前で人間に質問する', 'エージェントB', '待てないことだけに使います。確認、権限、人間にしかできない操作です。ダイアログはosascriptが別プロセスで描きます。全ウィンドウの手前に、音つきで、権限は要りません。90秒以内の回答はその場で返ります。'],
    ['人間が席にいない：回答は後からターンとして届く', '待機', '90秒でコマンドはPENDINGを返し、セッションは先へ進みます。ダイアログは開いたまま（既定で6時間）で、クリックの結果はセッションのsocketか受信箱へ入ります。Cancel、Esc、期限切れならCANCELLED / EXPIRED。質問は繰り返さず、ボードに載せます。'],
  ...
  const TERM = {
    ...
    '# что делает оркестратор: longrun send сессии, или предлагает человеку interrupt': '# オーケストレーターがすること：そのセッションへlongrun send、または人間にinterruptを提案',
