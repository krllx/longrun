...
(function (C) {
  ...
  Object.assign(N.SA, { title: 'A 的自有', sub: ['notes.md', 'journal, meta'] });
  Object.assign(N.SB, { title: 'B 的自有', sub: ['notes.md', 'journal, meta'] });
  ...
  const PF = {
    ...
    'archive/precompact/<sid>-<time>.md': ['compaction 之前的快照：用户最近的几个请求、编辑过的文件、最近的 FAIL。HANDOFF 区块（compaction 之后交接给自己）就是用它拼出来的。', 'PreCompact hook'],
    'archive/compact/<sid>-<time>.md': ['compaction 总结的原文。longrun recall 能搜到它们。', 'PostCompact hook'],
  ...
  const OF = {
    'aaaaaaaa/meta.json': ['轮次和调用计数、cwd、最后一次回答、共享笔记里哪些已经给它看过、遥测（正在跑的工具、是否在等）。', '每个事件上的 hook'],
    'aaaaaaaa/journal.md': ['里程碑 (longrun log) 和机械记录：启动、结束、失败的命令、compaction、发送。', 'hook 和 longrun log'],
    'aaaaaaaa/notes.md': ['自有笔记 [s1]、[s2]…… 预算 3000 字节。', 'agent：add, rm, replace, prune'],
    'aaaaaaaa/asks.json': ['这个会话问过人的问题：Q<n>、状态、回答。', 'longrun ask'],
    'bbbbbbbb/meta.json': ['会话 B 的同一份东西。', 'hook'],
  ...
  const GF = {
    'watch/w1.json': ['一个延迟检查：检查什么、告诉谁、告诉什么、什么时候过期。', 'watch add；定时器的每次 tick 更新它'],
    'halt.json': ['对所有项目的所有会话生效的停止：谁停的、什么时候、为什么。', 'halt / resume，预算监视器'],
  ...
  steps(C.s1, [
    ['文件夹变成项目', '人或 agent', '只做一次，在项目文件夹里，也就是 Claude Code 打开的那个。多出一个 <b>.longrun/</b> 目录：这就是“项目”，是这个文件夹下所有会话的共享记忆。'],
    ['会话 A 启动：SessionStart hook 打印摘要', 'hook', 'hook 沿目录树向上找到项目（找 .longrun/），在仓库之外建好会话目录，再把摘要打印进模型的上下文。现在还是空的。'],
    ['一条死胡同：agent 给自己记下来', 'agent', '测试挂住了，原因找到了。compaction 最先丢的就是这个，所以马上写一行到磁盘上。不加 <code>--shared</code> 的笔记是自有的：<b>[s1]</b>。'],
    ['一条大家都要的事实：写进共享笔记', 'agent', 'PR 号和类型上的决定，项目里任何一个会话都用得上。<code>--shared</code> 把它们写进项目的 NOTES.md：<b>[n1]</b>、<b>[n2]</b>，带上作者标记。'],
  ...
  steps(C.s2, [
    ...
    ['预算是硬的：add 会拒绝，prune 来清理', 'agent', '共享笔记 5000 字节，自有笔记 3000。文件满了不会悄悄挤掉旧行：<code>add</code> 直接拒绝，并指出删掉哪几条最划算。习惯是：做完的用 <code>rm</code> 删掉，用 <code>replace</code> 而不是再加一行，摘要里出现 PRUNE NEEDED 就 <code>prune</code>。'],
  ]);
  steps(C.s3, [
    ['A 把活交给 B：收件方已经停了', 'agent A', '收件方的名字就是人看到的样子：侧边栏标题、注册表里的名字，或者 id 前缀。B 现在没在运行，所以这段文字以文件形式落进项目的 <b>inbox</b>。'],
    ['B 醒来：消息作为一轮送到', 'hook', 'B 的第一个 hook（一轮输入、一次工具调用或一次启动）把文件从收件箱取走，打印进上下文。对模型来说这是同伴发来的请求，不是人下的指令：它的权限还是它自己的。'],
    ...
    ['等一个事件：用 watch，不要轮询', 'agent A', '“两小时后看一下”、“PR 合了以后”：不用 sleep 循环，也不用提醒人。注册的是一个确定性的检查。<code>add</code> 会立刻先试一次：已经成立就什么都不注册，直接去做。'],
    ['定时器每 5 分钟检查一次，不调用模型', '定时器', '一次 tick 不花 token。条件不成立就更新记录，仅此而已。笔记本睡着了，醒来会补上。连着三次硬错误或者过了截止时间，会发来一条 "broken" 或 "expired"。'],
  ...
  steps(C.s4, [
    ['A 接过编排器的角色', 'agent A，按人的吩咐', '一个项目由一个会话来推动目标。它拿到一份职责清单，而且<b>什么都不轮询</b>：事件会叫醒它。第二个会话想接会被拒绝，并看到现在是谁拿着。'],
    ['任务板：任务和派发', '编排器', '任务 T<n>，状态有 todo / doing / blocked / done / dropped。<code>assign</code> 把任务送进活着的会话的 socket，或者送进已停会话的收件箱。'],
    ['B 把任务当作一轮收下', 'hook', '干活的会话只需要三条命令：<code>take</code>、<code>done</code>、<code>block</code>。别的什么都不用学。'],
    ...
    ['编排器醒来并处置', '编排器', '它读任务板和事实，不读 transcript：自己的上下文必须保持小。每一条事实都要明确收尾：转达出去、重新规划，或者去问人。'],
    ['不花 token 的遥测：SESSIONS 里的标记', 'hook 和监视器', 'PreToolUse、PostToolUse、Stop、PermissionRequest 和 Notification 这几个 hook 把工具何时开始、何时结束、会话是否在等权限写进 meta.json。监视器（定时器）每 5 分钟和阈值比一次，每个事件段只向编排器报一次。'],
    ['全部停止：halt', '编排器，按人的吩咐', 'halt.json 文件是全局的。每个会话的 <b>PreToolUse</b> hook 都会带着原因回一个拒绝；模型看得到，会告诉人它本来打算做什么。只有人能解除，而且是在对话里用话说出来：<code>longrun resume</code>。'],
    ['盖在所有窗口之上问人一个问题', 'agent B', '只用于等不了的事：一次确认、一个权限、一件只有人能做的事。对话框由 osascript 在一个单独的进程里画出来：盖在所有窗口之上，带提示音，不需要任何权限。90 秒之内的回答会直接返回。'],
    ['人不在：回答稍后作为一轮送到', '等待', '90 秒之后命令返回 PENDING，会话继续往下走。对话框还开着（默认 6 小时）；点击的结果会进会话的 socket 或收件箱。Cancel、Esc 或过期：CANCELLED / EXPIRED，问题不再重复问，而是挂到任务板上。'],
    ['5 小时窗口的预算', '监视器', '监视器每 5 分钟读一次窗口用量（用 Claude Code 的 token 调 usage 接口），并和计划比一比：每小时 20%。超出 1.5 倍：所有项目全部停止、向编排器汇报，并给人弹一个 Keep stopped / Resume all 对话框。'],
  ]);
  ...
  const TERM = {
    ...
    '# при стопе: HALT + уведомление macOS + диалог Keep stopped / Resume all; `longrun resume` снимает и усыпляет правило до сброса окна': '# 停止时：HALT + macOS 通知 + Keep stopped / Resume all 对话框；`longrun resume` 解除停止，并把这条规则休眠到窗口重置',
  };
