...
(function (C) {
  ...
  Object.assign(N.L, { title: '定时器，每 5 分钟', sub: ['watch，监视器'] });
  ...
  const PF = {
    ...
    'board.json': ['编排器的任务板：目标、任务 T<n>、事实 F<n>。', 'board'],
  ...
  const OF = {
    ...
    'aaaaaaaa/journal.md': ['机械性的记录：启动、结束、失败的命令、compaction、发出去的消息。由 hook 写，不是 agent 写。', 'hook'],
    ...
    'bbbbbbbb/journal.md': ['会话 B 的日志。', 'hook'],
  ...
  const GF = {
    ...
    'halt.json': ['全部停止的记录，对所有项目的所有会话生效：谁停的、什么时候、为什么。只有人能解除。', 'halt / resume'],
  };
  ...
  steps(C.s2, [
    ...
    ['B 弄清了一条环境事实并分享出去', 'agent B', 'Stripe key 不在 env 文件说的那个地方。这是花了力气才弄清的，而且谁都用得上，所以只要 <code>-t fact</code>，什么参数都不用加：默认就写进共享笔记。'],
    ...
    ['已经不成立了：用 stale，而不是悄悄删掉', 'agent B', '那一行是别人写的，背着人家删掉不合适。<code>stale</code> 给它打上标记：条目照样显示，还带着原因，下一次清理先拿打了标记的条目开刀。<code>mute</code> 是另一回事：它只把这一行从<b>你自己</b>的摘要里去掉，对别人什么都不改。'],
  ]);
  steps(C.s3, [
  ...
  steps(C.s4, [
    ['A 接过编排器的角色', 'agent A，按人的吩咐', '每个项目有一个会话拿着任务板，并负责去找人。这不是自治地朝着某个目标走：把一个会话推向一个可检验的条件，那是内置的 <code>/goal</code>，它有真正的评估器。这里说的是另一回事：好几个窗口、它们中间的一块任务板，以及一条随时能问到人的通道。这个角色<b>什么都不轮询</b>：事件会叫醒它。第二个会话想接会被拒绝，并看到现在是谁拿着。'],
    ...
    ['B 干完活并关掉任务；一条来自外部的事实', 'agent B', '<code>done</code> 和 <code>block</code> 会叫醒编排器。评审要求改一个字段名：这是<b>来自外部的事实</b>。规矩是：先记下来，再做决定。事实和任务一样，都是同一块任务板上的条目，所以用 <code>board add --fact</code>；它同样会送到编排器那儿。'],
    ['编排器醒来并处置', '编排器', '它读任务板和事实，不读 transcript：自己的上下文必须保持精简。每一条事实都要明确收尾：转达出去、重新规划，或者去问人。'],
    ['不花 token 的遥测：SESSIONS 里的标记', 'hook 和监视器', 'PreToolUse、PostToolUse、Stop、PermissionRequest 和 Notification 这几个 hook 把工具何时开始、何时结束、会话是否在等权限写进 meta.json。监视器（定时器）每 5 分钟检查两个阈值：一个还在跑的工具，一个没人回答的权限弹窗，同一件事只向编排器报一次。其余的标记（一轮特别长、连着好几次 FAIL、上下文快到窗口）它只显示出来：那是正常干活，不是毛病。'],
    ['全部停止：halt', '编排器，按人的吩咐', '这是唯一一个真能让活停下来的开关，而且要人发话才拉。halt.json 文件是全局的。每个会话的 <b>PreToolUse</b> hook 都会拒绝执行并附上原因；模型看得到，会告诉人它本来打算做什么。只有人能解除，而且要在对话里明说：<code>longrun resume</code>。'],
  ...
  const TERM = {
    ...
    '# в дайджесте любой сессии эта строка теперь выглядит так:': '# 在任何一个会话的摘要里，这一行现在长这样：',
    '# отчет смотрителя оркестратору, один раз на эпизод:': '# 监视器给编排器的报告，同一件事只报一次：',
    '# ничего не убивается: остановить окно может только человек': '# 什么都不会被杀掉：只有人能停下那个窗口',
