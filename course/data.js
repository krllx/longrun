...
window.COURSE = (function () {
  const FOOT = 'Write: `longrun add -t dead|decision|fact|ctx|pin "<one line>"` (shared with every session of the project; `--own` keeps it here). Lost detail: `longrun recall <term>`. Others: `longrun status`, `longrun send <name> "..."`. Wait: `longrun watch add --then "..." -- pr-merged <id>`, never poll. `longrun help`.';
  ...
  const N = {
    ...
    L: { id: 'L', kind: 'timer', x: 230, y: 250, w: 170, h: 50, title: 'таймер, раз в 5 мин', sub: ['watch, смотритель'] },
  ...
  const PROJ = { label: 'shop/.longrun/  (общее, в проекте)', prefix: 'shop/.longrun/', files: [
    { path: 'NOTES.md', what: 'Общие заметки [n1], [n2]... одной строкой, с автором. Бюджет 5000 байт. Сюда заметка идет по умолчанию.', who: 'Пишет агент: add, rm, replace, stale, prune --shared' },
    { path: 'config.json', what: 'Настройки проекта: бюджеты заметок и дайджеста, пороги.', who: 'init; longrun config set' },
    { path: 'state.json', what: 'Счетчики id заметок и задач.', who: 'CLI' },
    { path: 'stale.json', what: 'Пометки "перестало быть правдой": id -> причина, кто и когда. Лежит рядом с NOTES.md, а не внутри: формат заметок не меняется.', who: 'longrun stale' },
    ...
    { path: 'board.json', what: 'Доска оркестратора: цель, задачи T<n>, факты F<n>.', who: 'board' },
  ...
  const OWN = { label: '~/.claude/longrun/sessions/shop/<сессия>/  (свое, вне репозитория)', prefix: '~/.claude/longrun/sessions/shop/', files: [
    ...
    { path: 'aaaaaaaa/journal.md', what: 'Механические строки: старт, конец, FAIL, компакция, отправки. Пишут хуки, не агент.', who: 'хуки' },
    ...
    { path: 'bbbbbbbb/journal.md', what: 'Журнал сессии B.', who: 'хуки' },
  ...
  const GLOB = { label: '~/.claude/longrun/  (глобальное)', prefix: '~/.claude/longrun/', files: [
    ...
    { path: 'halt.json', what: 'Стоп для всех сессий всех проектов: кто, когда, почему. Снимает только человек.', who: 'halt / resume' },
  ] };
  ...
  const s1 = Object.assign(pick(['U', 'A', 'P', 'SA'], ['AP', 'PA', 'ASA', 'SAA', 'UA']), { size: [560, 215], roots: [PROJ, OWN], steps: [
    ...
    { title: 'Сессия A стартует: хук SessionStart печатает дайджест', who: 'хук', nodes: ['A', 'P', 'SA'], edges: ['PA', 'SAA'],
      ...
      term: [{ dim: '# SessionStart (source=startup) -> в контекст модели:' }, { out: '<longrun v0.6.1 project=shop session=aaaaaaaa [aaaaaaaa] source=startup>\nSHARED notes (project shop) 0 entries 0/5000 B (~0 tok)\nOWN notes (this session) 0 entries 0/3000 B (~0 tok)\n' + FOOT + '\n</longrun>' }],
    ...
    { title: 'Тупик: агент записывает его себе', who: 'агент', nodes: ['A', 'SA'], edges: ['ASA'],
      caption: 'Тест повис, причина найдена. Это первое, что теряет компакция, поэтому одна строка на диск сразу. Тупик здесь про эту ветку и никому больше не нужен, поэтому <code>--own</code>: заметка своя, <b>[s1]</b>.',
      term: [{ who: 'A', cmd: 'longrun add --own -t dead "pytest -x hangs on test_checkout: it waits for a real Redis; run with REDIS_URL=fake://"' }, { out: 'added s1 (own, 107/3000 bytes, ~26 tokens)' }],
      files: [['aaaaaaaa/notes.md', 'new']] },
    { title: 'Факт для всех: в общие заметки', who: 'агент', nodes: ['A', 'P'], edges: ['AP'],
      caption: 'Номер PR и решение о типе нужны любой сессии проекта, поэтому флага не нужно: <code>add</code> пишет в NOTES.md проекта по умолчанию - <b>[n1]</b>, <b>[n2]</b>, с меткой автора.',
      term: [{ who: 'A', cmd: 'longrun add -t pin "PR 42 = feature/cart-total, branch cart-total"' }, { out: 'added n1 (shared, 71/5000 bytes, ~17 tokens)' }, { who: 'A', cmd: 'longrun add -t decision "Round prices with Decimal, not float: float gave 19.999999 in the cart"' }, { out: 'added n2 (shared, 172/5000 bytes, ~43 tokens)' }, { who: 'A', cmd: 'cat .longrun/NOTES.md' }, { out: '# longrun notes (one line per entry; edit with `longrun add|rm|replace|prune`)\n- [n1] 09-10 pin (shop): PR 42 = feature/cart-total, branch cart-total\n- [n2] 09-10 decision (shop): Round prices with Decimal, not float: float gave 19.999999 in the cart' }],
      files: [['NOTES.md', 'chg']] },
    { title: 'Длинное в строку не влезает: это файл', who: 'агент', nodes: ['A', 'P'], edges: ['AP'],
      caption: 'План раскатки не сожмешь в 400 символов. Он остается файлом в проекте, а в общих заметках стоит одна строка с путем: <b>[n3]</b>. Каждая сессия видит строку и открывает файл, только когда он ей нужен. Файл поменялся после записи строки - дайджест это пометит.',
      term: [{ who: 'A', cmd: 'longrun doc add research/rollout.md "the rollout plan and what is still open"' }, { out: 'added n3 (shared, 259/5000 bytes, ~64 tokens)' }, { who: 'A', cmd: 'longrun doc ls' }, { out: 'n3  research/rollout.md  1.4 KB, changed 09-10  the rollout plan and what is still open' }],
      files: [['NOTES.md', 'chg']] },
    { title: 'Компакция: механика снимает и архивирует', who: 'хуки', nodes: ['A', 'P', 'SA'], edges: ['AP', 'ASA'],
      ...
      term: [{ dim: '# PreCompact (trigger=auto), PostCompact -> файлы, вывода в контекст нет' }, { who: 'A', cmd: 'tail -3 ~/.claude/longrun/sessions/shop/aaaaaaaa/journal.md' }, { out: '09-10 11:03 compaction auto | snapshot archive/precompact/aaaaaaaa-20260910-110357.md\n09-10 11:03 session compact ()' }],
